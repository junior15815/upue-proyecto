...
import pe.edu.upeu.modelo.VentaDetalleJV;
import pe.edu.upeu.modelo.VentaJV;
...
public class Venta extends AppCrud{
   LeerArchivo lar;
   ...
   Ansi color=new Ansi();
    ...
    public void registroVentaGeneral() {
        VentaJV vTO=crearVenta();
        double precioTotalX=0;
        String continuaV="SI";
        do {
            VentaDetalleJV toVD=carritoVenta(vTO);
    ...
    public VentaDetalleJV carritoVenta(VentaJV vTO) {
        
        vdTO=new VentaDetalleJV();
        ut.clearConsole();
        ...
        vdTO.setPorcentUtil(0);
        vdTO.setPrecioUnit(punit);
    ...
    public void mostrarProductos() {
        ...
        for (int i = 0; i < dataP.length; i++) {
            ...
            "(P.Unit S/.:"+dataP[i][4]+", Stock:"+dataP[i][5]+"); ");
            System.out.println("");
        }
        System.out.println("");
    }
    /*public double ofertaVentas(String dni, double MCactual){
        lar=new LeerArchivo("Venta.txt");
        Object[][] dataP=buscarContenido(lar,1, dni);
